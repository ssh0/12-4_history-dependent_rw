...
class HistoryDependentRW():
    ...
    def random_walk_d1(self, N):
        ...
        cdef np.ndarray[np.int32_t, ndim = 2] x = np.zeros([self.nwalkers, max(N)], 'i')
        cdef np.ndarray[np.int32_t, ndim = 2] x_direction = np.zeros([self.nwalkers, max(N)], 'i')
        cdef np.ndarray[np.float64_t, ndim = 2] p = np.random.random([self.nwalkers, max(N) - 1])  # generate random number in [0,1)
        cdef double alpha
        cdef int x0
        alpha = self.alpha
        l = self.l
